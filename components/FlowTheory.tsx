...
import { Waves, Loader2, Copy, Check, Link as LinkIcon, Shuffle, Zap, Target, FileText, BookOpen, Globe } from 'lucide-react';
...
const FlowTheory: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    setAnalysis(null);
    try {
      const result = await generateFlowAnalysis(gameName, gameplay, storeUrl, playerSkill, language);
      setAnalysis(result);
    } catch (error) {
      console.error(error);
      alert("生成分析失败，请重试。");
