...
import { MonitorPlay, Loader2, Copy, Check, FileText, Globe, Shuffle, Zap } from 'lucide-react';
import { generateIaaPlan, analyzeGameplayFromUrl } from '../services/geminiService';
import { exportToGoogleDocs } from '../utils/exportUtils';

const IaaMonetization: React.FC = () => {
  ...
  const [targetMarket, setTargetMarket] = useState('Global (Tier 1 Focus)');
  ...
  const handleGenerate = async () => {
    ...
    setPlan(null);
    try {
      const generatedPlan = await generateIaaPlan(gameName, genre, gameplay, targetMarket, language);
  ...
  const handleExport = () => {
    if (!plan) return;
    exportToGoogleDocs(plan, `IAA Monetization Strategy - ${gameName}`);
  };
    ...
    <div className="flex h-full gap-6">
      ...
      <div className="w-1/3 bg-slate-800 rounded-xl p-6 border border-slate-700/50 flex flex-col shrink-0">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-400 mt-1">设计基于广告变现的混合或纯 IAA 策略。</p>
        </div>
        ...
        <div className="space-y-5 flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="p-4 bg-indigo-900/20 rounded-lg border border-indigo-500/20">
             <h4 className="text-indigo-300 font-bold text-xs mb-2 flex items-center gap-1">
               <Zap className="w-3 h-3" /> 广告策略重点
             </h4>
             <ul className="text-xs text-slate-400 space-y-2 list-disc pl-4">
               <li><strong className="text-slate-300">广告形式:</strong> 激励视频、插屏、Banner、原生广告。</li>
               <li><strong className="text-slate-300">展示逻辑:</strong> 频率控制、展示时机、冷却时间。</li>
               <li><strong className="text-slate-300">混合变现:</strong> 如何平衡广告与内购，避免用户流失。</li>
             </ul>
          </div>
        </div>
        ...
        {plan ? (
          <>
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">IAA 商业化策略方案</h2>
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-500">
             ...
             <p className="text-lg font-medium">IAA 策略规划</p>
             <p className="text-sm max-w-xs text-center mt-2">输入游戏详情，AI 将为您设计最佳的广告变现组合与展示逻辑。</p>
          </div>
        )}
      </div>
    </div>
...
export default IaaMonetization;
