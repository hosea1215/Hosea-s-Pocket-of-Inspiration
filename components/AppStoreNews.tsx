...
import { Newspaper, Loader2, Copy, Check, FileText, Globe, ExternalLink, RefreshCw, Apple } from 'lucide-react';
import { generateAppStoreNews } from '../services/geminiService';
import { exportToGoogleDocs } from '../utils/exportUtils';

const AppStoreNews: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    setNews(null);
    try {
      const result = await generateAppStoreNews(language);
      setNews(result);
    } catch (error) {
      console.error(error);
      alert("获取资讯失败，请重试。");
  ...
  const handleExport = () => {
    if (!news) return;
    exportToGoogleDocs(news, `App Store News Briefing - ${new Date().toLocaleDateString()}`);
  };
    ...
    <div className="flex h-full gap-6">
      ...
      <div className="w-1/3 bg-slate-800 rounded-xl p-6 border border-slate-700/50 flex flex-col shrink-0">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-400 mt-1">获取最近 30 天 iOS App Store 政策、ASO 与市场动态简报。</p>
        </div>
        ...
        <div className="space-y-5 flex-1 overflow-y-auto custom-scrollbar">
          ...
          <div className="p-4 bg-indigo-900/20 rounded-lg border border-indigo-500/20">
             ...
             <ul className="text-xs text-slate-400 space-y-2 list-disc pl-4">
               <li><strong className="text-slate-300">Policy Updates:</strong> App Review 指南更新、隐私政策。</li>
               <li><strong className="text-slate-300">ASO & Search:</strong> 搜索广告 (ASA)、关键词排名机制。</li>
               <li><strong className="text-slate-300">Features:</strong> App Store Connect 更新、新硬件适配。</li>
               <li><strong className="text-slate-300">Trends:</strong> 榜单变化、iOS 版本特性。</li>
             </ul>
          </div>
        </div>
        ...
        {news ? (
          <>
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-700">
              <h2 className="text-xl font-bold text-white">App Store 资讯简报</h2>
          ...
          <div className="flex flex-col items-center justify-center h-full text-slate-500">
             ...
             <p className="text-sm max-w-xs text-center mt-2">AI 将实时检索网络，为您汇总 App Store 的最新动态。</p>
          </div>
        )}
      </div>
    </div>
