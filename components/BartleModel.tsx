...
import { Users, Loader2, Copy, Check, Link as LinkIcon, Shuffle, Trophy, Map, MessageCircle, Crosshair, FileText, BookOpen } from 'lucide-react';
...
const BartleModel: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    setResult(null);
    try {
      const data = await generateBartleAnalysis(gameName, gameplay, storeUrl);
      setResult(data);
    } catch (error) {
      console.error(error);
      alert("生成分析失败，请重试。");
